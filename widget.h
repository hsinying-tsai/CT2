...
private slots:
    ...
    void WR_command(QString WR_message); //處理傳送指令至PLC
    void on_puB_sent_clicked(); 
    void on_puB_read_clicked();
    void on_puB_write_clicked();
    void recv_label_update(QString message); //處理來自PLC的指令
    void connect_label_update(); //socket通訊狀態改變

    void on_puB_runMode_clicked(); //可選擇自動/手動 模式
    void on_puB_saveINI_clicked(); //很少用到,功能為可在tabWidget_2修改recipe.ini
    void imagesprocess();
    void showDetectFlow(QLabel *label); //顯示那五條檢測流程
